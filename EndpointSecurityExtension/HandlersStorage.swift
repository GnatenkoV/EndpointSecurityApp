...
class HandlersStorage {
    ...
    public static func onAuthFileOpenReadonlyConfig(_ client : OpaquePointer) -> Void
    {
        // clear all paths
        es_unmute_all_target_paths(client)
        
        // invert muting behavior
        es_invert_muting(client, ES_MUTE_INVERSION_TYPE_TARGET_PATH)
        
        // unmute the target path prefix
        es_mute_path(client, "/Users/user/Downloads", ES_MUTE_PATH_TYPE_TARGET_PREFIX)
    }
    
    public static func onAuthFileOpenReadonly(_ client : OpaquePointer, message : UnsafePointer<es_message_t>) -> Bool
    {
        let readonlyBoundleIds : [NSString] = ["com.barebones.bbedit"]
        
        let protectedPaths = ["/Users/user/Downloads"]
        
        let filePath = NSString(bytes: message.pointee.event.open.file.pointee.path.data, length: Int(message.pointee.event.open.file.pointee.path.length), encoding: NSASCIIStringEncoding)
        ...
        for i in 0..<protectedPaths.count
        {
            ...
            {
                os_log(OSLogType.info, "[client2] Trying to get folder access %{public}@", filePath!)
                ...
                os_log(OSLogType.info, "[client2] Checking access for %{public}@", boundleId!)
                
                if (readonlyBoundleIds.contains(boundleId!))
                {
                    os_log(OSLogType.info, "[client2] Access readonly for %{public}@", boundleId!)
    ...
    public static func onAuthFileOpenDenieAccessConfig(_ client : OpaquePointer) -> Void
    {
        es_unmute_all_target_paths(client)
        es_invert_muting(client, ES_MUTE_INVERSION_TYPE_TARGET_PATH)
        es_mute_path(client, "/Users/user/Downloads", ES_MUTE_PATH_TYPE_TARGET_PREFIX)
    ...
    public static func onAuthFileOpenDenieAccess(_ client : OpaquePointer, message : UnsafePointer<es_message_t>) -> Bool
    {
        ...
        os_log(OSLogType.info, "[client3] Folder access %{public}@", filePath!)
        ...
        for i in 0..<protectedPaths.count
        {
            ...
            if (filePath!.contains(protectedPath))
            {
                os_log(OSLogType.info, "[client3] Trying to get access for %{public}@", filePath!)
                
                let signing_id = message.pointee.process.pointee.signing_id
                let boundleId = NSString(bytes: signing_id.data, length: Int(signing_id.length), encoding: NSASCIIStringEncoding)
             
                os_log(OSLogType.info, "[client3] Checking access for boundleId:%{public}@", boundleId!)
                
                if (!allowedBoundleIds.contains(boundleId!))
                {
                    os_log(OSLogType.info, "[client3] Access denied for %{public}@ to %{public}@", boundleId!, filePath!)
